...
var CFAlertDialog = (function () {
    ...
    CFAlertDialog.prototype.show = function (options) {
        ...
        if (options.backgroundColor)
            builder.setBackgroundColor(new core_1.Color(options.backgroundColor).android);
        if (options.textColor)
            builder.setTextColor(new core_1.Color(options.textColor).android);
        ...
        return new Promise(function (resolve, _) {
            if (options.buttons) {
                options.buttons.forEach(function (button) {
                    builder.addButton(button.text, button.textColor ? new core_1.Color(button.textColor).android : -1, button.backgroundColor ? new core_1.Color(button.backgroundColor).android : -1, actionStyles[button.buttonStyle], alignment[button.buttonAlignment], new android.content.DialogInterface.OnClickListener({
            ...
            if (options.titleColor) {
                _this._alertDialog.setTitleColor(new core_1.Color(options.titleColor).android);
            }
            if (options.messageColor) {
                _this._alertDialog.setMessageColor(new core_1.Color(options.messageColor).android);
            }
